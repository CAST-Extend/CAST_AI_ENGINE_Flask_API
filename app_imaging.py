...
class AppImaging:
    ...
    def get_source(self, object_type, tenant, application, object_id,  start_line, end_line):
        ...
        if object_code_response.status_code == 200:
            ...
            object_code = ""
            self.app_logger.log_error("get_source", f"Failed to fetch {object_type} code using {object_code_url}. Status code: {object_code_response.status_code}")
        return object_code

    def get_file(self, object_type, tenant, application, file_id):
        ...
        if object_code_response.status_code == 200:
            ...
            object_code = ""
            self.app_logger.log_error("get_file", f"Failed to fetch {object_type} code using {object_code_url}. Status code: {object_code_response.status_code}")
        return object_code
